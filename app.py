...
input[type=text],textarea,.stTextInput input,.stSelectbox select{background:#111827!important;border:1px solid #1e2d4a!important;color:#e2e8f0!important;border-radius:8px!important;font-size:.8rem!important}
...
def kpi_cards(alerts):
    counts = {s: sum(1 for a in alerts if a.get("severity","").startswith(s[:3])) for s in ["Critical","High","Medium","Low"]}
    defs = [
...
def alert_card(a, idx):
    ...
    cls  = SEV_CLS.get(sev, "C")
    dot  = SEV_DOT.get(sev, "⚪")
    risk = a.get("risk_score", a.get("score", "N/A"))
    tech = a.get("mitre_technique", a.get("technique","—"))
    ts   = a.get("timestamp", a.get("time","—"))
    det  = a.get("final_detection", a.get("detection", a.get("description","—")))
    threat = a.get("threat", a.get("alert_name", a.get("event_type","Unknown Threat")))
    ...
    <div class="card" style="--ac:{ac}">
      ...
      <div class="meta">
        ...
        <span class="chip">🎯 {a.get("mitre_tactic","—")}</span>
      </div>
...
def main():
    ...
    sevs  = ["All"] + [s for s in ["Critical","High","Medium","Low"] if any(a.get("severity","").startswith(s[:3]) for a in alerts)]
    ...
    filtered = [a for a in alerts if
        (sf == "All" or a.get("severity","").startswith(sf[:3])) and
